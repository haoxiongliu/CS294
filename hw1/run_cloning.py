...
def main():
    ...
    with tf.Session() as sess:
        ...
        # train
        EPOCH = len(expert_observations)
        num_of_epoch = 1
        for i in range(num_of_epoch*EPOCH):
            train_step.run(feed_dict={ex_obs: expert_observations[i % EPOCH],
                                      ex_action: expert_actions[i % EPOCH][0]})
